...
function process(sqsConfig, event, context) {
    ...
    if (sqsConfig.routes && sqsConfig.routes.length > 0) {
        const result = sqsConfig.routes[0].action(event.Records, context);
        return result || {};
    }

    if (sqsConfig.debug) {
        console.log('No SQS matcher defined');
    }
...
const cfgExample = {
    routes:[
        {
            action: (records, context) => service.import(JSON.parse(records[0].body), context)
        }
    ]
};
*/
