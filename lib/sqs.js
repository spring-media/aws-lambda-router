...
function process(sqsConfig, event, context) {
    ...
    for (let routeConfig of sqsConfig.routes) {
        if (routeConfig.source instanceof RegExp) {
            if (routeConfig.source.test(records[0].eventSourceARN)) {
                const result = routeConfig.action(records.map(record => record.body) , context);
                return result || {};
            }
        } else {
            if (routeConfig.source === records[0].eventSourceARN) {
                const result = routeConfig.action(records.map(record => record.body) , context);
                return result || {};
            }
        }
    ...
    if (sqsConfig.debug) {
        console.log(`No source-match for ${records[0].eventSourceARN}`);
    }
