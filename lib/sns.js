...
function process(snsConfig, event, context) {
    ...
    for (let routeConfig of snsConfig.routes) {
        if (routeConfig.subject instanceof RegExp) {
            const recordsMatchingToRoute = records.filter(record => routeConfig.subject.test(record.Sns.Subject));
            const snsMessages = recordsMatchingToRoute.map(record => record.Sns);

            if (snsConfig.debug) {
                console.log(`${snsMessages.length} SNS messages match to route config ${routeConfig.subject}`);
            }

            if (snsMessages.length < 1) {
                continue;
            }

            const result = routeConfig.action(snsMessages, context);
            console.log("action result ignored ", result);
        } else {
